...
//	dcb.fRtsControl = RTS_CONTROL_DISABLE;
	...
	if(!SetCommState (m_hCommPort, &dcb)) {
		printf("Port open error %d (3)\n",GetLastError());
		return -1;
	}

	COMMTIMEOUTS to; 
	if(!GetCommTimeouts(m_hCommPort,&to)) {
		printf("Port open error %d (4)\n",GetLastError());
		return -1;
	}
