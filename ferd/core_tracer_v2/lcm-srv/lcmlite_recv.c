...
static void feex_callback(lcmlite_t *lcm, const char *channel, const void *buf, int buf_len, void *user)
{
    ...
    if(buf_len > 10) {
	lcm_recvd((uint8_t *)buf+10,buf_len-10);
    }
}
...
//        return -1;
	...
	if(buf_len <= 0) continue;
        ...
        if (res < 0) {
            printf("lcm recieve packet error %d\n", res);
	}
    }
    return 0;
}
...
{
uint8_t buf2[8*1024] = {
    0x64, 0x29, 0x5E, 0x90, 0xFD, 0x64, 0xAC, 0x86, // fingerprint
    (len >> 8) & 0xff,
    len & 0xff
};
    memcpy(buf2+10,buf,len);
    lcmlite_publish(&lcm, "FEEX.MEAS", buf2, len+10);
}
