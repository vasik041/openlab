...
public class Graph extends JPanel {
		...
		if(sig2 != null) {
			...
			for(int i=0; i < signal.length-1; i++) {
				g.drawLine(5+i*(w-10)/signal.length,
						h/2-5-(int)((double)(h/2-10)*((signal[i]-min2)/amp2)),
						5+(i+1)*(w-10)/signal.length,
						h/2-5-(int)((double)(h/2-10)*((signal[i+1]-min2)/amp2)));
			}
		}
	...
	private void paintComponentXY(Graphics g) {
		...
		if(sig1 != null && sig2 != null) {
			...
			double max2 = sig2.getMax();
			double min2 = sig2.getMin();
			double amp2 = max2-min2;
			...
			for(int i=0; i < signal.length-1; i++) {
				...
				int y =  (int)((double)(h-10)*((signal2[i]-min2)/amp2));
				int y2 = (int)((double)(h-10)*((signal2[i+1]-min2)/amp2));
